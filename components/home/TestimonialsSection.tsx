...
function TestimonialCard({ testimonial, featured }: { testimonial: Testimonial; featured: boolean }) {
  return (
    <div
      className="card-hover rounded-2xl p-7 border flex flex-col bg-white"
      style={{
        borderColor: featured ? "rgba(200,169,110,0.4)" : "#e4e8ef",
        boxShadow: "0 10px 30px rgba(0,0,0,0.05)"
      }}
    >
      <StarRating />

      <blockquote className="font-body text-slate-700 text-sm leading-relaxed mb-6 flex-1 italic">
        "{testimonial.quote}"
      </blockquote>
